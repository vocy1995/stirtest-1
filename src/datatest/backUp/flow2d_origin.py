    ...
    for ind, dataset in enumerate(datasets[:num_data]):
        try:
            ...
            if not args.ontestingset: # Log endpoint error
                ...
                point_hutom = np.array([[446, 591],[468, 424]])
                errors_hutom  = pointlossunidirectional(pointlistend, point_hutom)
                print(f"hutom_model error : {errors_hutom['averagedistance']}")
                print(point_hutom)
                print(pointlistend)
